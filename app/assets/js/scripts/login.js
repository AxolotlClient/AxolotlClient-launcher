...
ipcRenderer.on('MSALoginWindowReply', (event, ...args) => {
    ...
    if (queryMap.has('error')) {
        ...
        if(error === 'access_denied'){
            error = 'ERROR'
            errorDesc = 'To use the AxolotlClient, you must agree to the required permissions! Otherwise you can\'t use this launcher with Microsoft accounts.<br><br>Despite agreeing to the permissions you don\'t give us the possibility to do anything with your account, because all data will always be sent back to you (the launcher) IMMEDIATELY and WITHOUT WAY.'
        }        
